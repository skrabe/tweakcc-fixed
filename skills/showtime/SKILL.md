...
# Upstream often publishes day-zero to a PER-VERSION branch, not main — try it first:
git show upstream/prompts/X.Y.Z:data/prompts/prompts-X.Y.Z.json > /tmp/upstream-X.Y.Z.json \
...
- **Sibling-check is mandatory, and it reads the DEPLOYED override — never pristine.**
  grep the rest of `system-prompts-<model>/` for any prompt conveying each claim; cut
  sentences already carried (more fully) by a sibling, keeping only the unique residue.
  **Coverage exists only in what the model actually receives**, so before accepting any
  "sibling Y covers this", check which of three cases Y is in:
  | Y's override file | What reaches the model | Does it cover? |
  | --- | --- | --- |
  | exists, has a body | **only that body** | only if the body carries the claim |
  | exists, body **empty** | nothing — Y is suppressed | **never** |
  | does not exist | Y's pristine, verbatim | yes, pristine coverage is real |
  Reading Y's pristine when Y has a trimmed override is the single most common way to
  delete information silently — the apply is clean and every counter stays zero.
  **Also resolve wipes before the trims that might cite them**, or you get circular
  coverage: A is trimmed because B covers it while B is emptied because A covers it,
  and neither carries the claim. See `REFERENCE.md → Bug class: a dedup claim checked
against pristine instead of the deployed override`.
...
4. **No latent var breakage** — orphan-`${VAR}` validator: `UNKNOWN_N placeholders: 0`
   ...
   **complete-but-mislabeled** maps too (named == distinct, names at wrong slots). Dump
   upstream first (per-version branch, falling back to main): `git show
   upstream/prompts/<ver>:data/prompts/prompts-<ver>.json > /tmp/upstream-<ver>.json ||
   git show upstream/main:data/prompts/prompts-<ver>.json > /tmp/upstream-<ver>.json`.
...
| **A dedup claim checked against pristine, not the deployed override**  | a trim justified by "sibling Y covers this" is FALSE when Y's own override trimmed that sentence out — the override REPLACES pristine, so only the deployed body reaches the model. Worse, running trims and wipes in one batch makes it circular: A is cut because B covers it while B is emptied because A covers it. Invisible to four-zeros, the mis-bind audit and smoke — the prompts apply cleanly; only the CONTENT is gone.                                                                         | Judge coverage on the deployed `.md` only (body → that body; empty body → suppressed, covers nothing; no file → pristine applies, so pristine coverage is real). Resolve wipes BEFORE the trims that might cite them, then adversarially re-verify every trim whose rationale cites a sibling. `REFERENCE.md → Bug class: a dedup claim checked against pristine instead of the deployed override`. |
| **A second builder appears for a region you patch**                    | upstream adds a new function alongside the old one and a gate selects between them (`cond ? [NEW(…)] : [OLD(…), …]`). Your override still matches its anchor in OLD and logs a clean `✓ … N fewer chars` — but OLD is no longer the branch that renders, so the patch is inert. A clean apply proves the ANCHOR matched, never that the patched branch is live.                                                                                                                                              | When a bump restructures a region, resolve the **selector** before trusting the placement: find the ternary and read the gate's definition (beware name collisions in a 20 MB bundle — search for the non-obvious assignment). Cross-check against the **running** system prompt, which is ground truth for which branch renders. Then re-home the override onto whatever owns the live branch.     |
| **A large id drop is not automatically a regression**                  | a release can delete a whole prompt subsystem at once (100+ ids). The no-regression bar still holds, but it needs per-id evidence, not a reflex "the extractor broke". Some ids also survive as text with a CHANGED FACING — moved from model context to a UI renderer — which is a legitimate drop, not a removal.                                                                                                                                                                                          | Probe each dropped id's pristine body against the new cli.js using ≥30-char literal runs (split on `${...}` first), in python not grep. Zero hits → real removal. Hits → read the **emission site**: model→UI is a legitimate drop, but report it as a facing change. Also grep the feature's identifiers — if those are gone too, the subsystem really went.                                       |
...
| **Backtick-hosted override needs PRE-DOUBLED backslashes**             | the prompt-splice escapes literal backslashes ONLY for the `"` and `'` delimiters (backticks are excluded so the parity-aware backtick pass isn't broken), so for a backtick emission site the override body must already contain `\\`. A single `\{` renders as `{` and `[\s\S]` as the literal class `[sS]`. Nothing catches it — apply is clean, the safety harness PASSes, every counter is zero; only reading the rendered text finds it. Seen shipping for many versions in a non-active override set. | Read the emission site's delimiter (print the ~200 chars BEFORE the content) before editing any override; backtick → double every `\`. Cheap smell: the same prompt written `\\{` in one set and `\{` in another.                                                                                                                                                                                   |
| **A gate that reads the WRONG instance of a repeated key**             | the version-bump report prints `anonymous:` once per metrics block (old / committed / extracted). A bare `/anonymous:\s*(\d+)/` matched the FIRST — the previous version's count — so the driver structurally reported the wrong number and printed `anonymous prompts: 0` on a bump that extracted 8, while the report itself listed all 8 as blocking.                                                                                                                                                     | Anchor on the BLOCK as well as the key whenever the key can repeat (`/extracted metrics:\s*\{[^}]*?anonymous:\s*(\d+)/s`). Prove such a fix against the real FAILING output — on a passing run both regexes agree and the test proves nothing.                                                                                                                                                      |
| **A file-writing agent that errors may have finished its writes**      | a fan-out stage that WRITES files reports failure from its return value, not from disk. On one run three of eight agents died with `Connection closed mid-response` — and every one of their files had already been written. Trusting the failure report means redoing work; trusting it the other way can ship truncated files.                                                                                                                                                                             | After any file-writing fan-out, reconcile the expected id list against DISK and structurally validate each file (frontmatter opens and closes; body non-empty iff the source was). Read the agents' summaries only after that.                                                                                                                                                                      |
| **Comparing an override against only the FIRST JSON entry for its id** | same-id multi-site prompts are normal — one id can occupy several binary sites with different reconstructed bodies. A stub-vs-real-override check that compares against `prompts.find(p => p.id === id)` misclassifies every file matching a LATER entry as a real override. This inflated a reported per-model backlog from 34 to 114.                                                                                                                                                                      | Build `id -> Set(all reconstructed bodies)` and test membership, never equality against one entry.                                                                                                                                                                                                                                                                                                  |
