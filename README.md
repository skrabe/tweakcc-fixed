...
> [!IMPORTANT]
> **This fork has permanently diverged from upstream (2026-06-04).** It is no longer "fixes waiting to merge back upstream." Piebald rewrote `tweakcc` into **v4.0.14**, which gates system-prompt overrides **off** for native installs and removes the inline-blob mechanism, the system-reminder mechanism, the Bun-bytecode-cache robustness, and several patches — i.e. it dropped the things this fork exists for. Upstream's source is now a strict subset of ours. We keep the `upstream` remote only as a **fetch-only comparison signal** (e.g. our extractor names 374 prompts for CC 2.1.162 vs their published 331) and never merge it.
...
| **Target CC versions** | 2.0.98 through **2.1.162**                                                                               |
...
Use this fork's extraction surface with [skrabe/lobotomized-claude-code](https://github.com/skrabe/lobotomized-claude-code) — overrides tuned for **Claude Opus 4.8** against this fork's extraction. Our named-prompt JSON catches prompts Piebald's published extract doesn't (374 named vs their 331 for CC 2.1.162), and the system-reminder + `string`-kind reach lets the overrides cover content the base mechanisms can't.
