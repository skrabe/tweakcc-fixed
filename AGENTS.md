...
### Override resolves to the WRONG var (mis-mapped slot — silent, no crash)

**Class.** The _silent sibling_ of the ReferenceError case above (and of the slot-shift class — showtime §12, memory `reference_identifier_slot_shifts_on_prompt_restructure`). Our `identifierMap` names **fewer slots than the binary actually uses**, so a reused named label binds to the wrong positional capture and resolves to a valid-but-wrong var → **no crash, wrong content**. Seen on `tool-description-croncreate` (2.1.168): our map named 3 of 7 detected slots, so `${CANCEL_TIMEFRAME_DAYS}` resolved to the `IS_MONITOR_TOOL_ENABLED` _function_ instead of the day-count number → the rendered description read "auto-expire after `function …` days". Invisible to four-zeros + smoke (lazy tool-description never materializes at boot); the `UNKNOWN_N` validator misses it because the override hardcodes/drops the unnamed slots instead of leaving `${UNKNOWN_N}`.

**Detection.** Self-contained — every distinct slot must be named. `jq '.prompts[] | select((.identifierMap|length) < (.identifiers|unique|length)) | .id' data/prompts/prompts-<ver>.json` must be empty. Cross-checking the same id against Piebald's `identifierMap` (`git show upstream/main:…`) confirms whether ours is partial.

**Fix.** Adopt the complete identifierMap for that prompt — name all slots via `NEW_PROMPT_ASSIGNMENTS`, or borrow Piebald's where it's fuller (confirm the `identifiers` array matches first) — then realign the override. We stay canonical for coverage but borrow Piebald's per-prompt logic when genuinely fresher: memory `feedback_diverge_from_piebald_but_adopt_better_logic`.
