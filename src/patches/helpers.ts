...
export const findBoxComponent = (fileContents: string): string | undefined => {
  ...
  // Method 6: Find Box by destructured signature with children, ref, tabIndex,
  // autoFocus (CC 2.1.126+). The 2.1.126 wrapper destructures focus/mouse handlers
  // explicitly before forwarding ...rest to createElement("ink-box", ...). Generic
  // fallback — runs only if the more specific rest-style match above misses.
