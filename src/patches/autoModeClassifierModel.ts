...
export const writeAutoModeClassifierModel = (
  ...
  // CC 2.1.177 shape: same prefix as 2.1.172 but the Fable/Mythos branch
  // collapsed from the verbose `ANTHROPIC_DEFAULT_OPUS_MODEL ?? opus48` logic to
  // a single resolver call:
  //   ...if(_?.model)return _.model;if(ISFABLE(H)||ISMYTHOS(H))return RESOLVE(H);return H}
  const pattern177 =
