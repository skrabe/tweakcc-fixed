...
export const writeAutoAcceptPlanMode = (oldFile: string): string | null => {
  ...
  // CC >=2.1.265: the dialog moved to the jsx runtime and the handler lost its
  // ref indirection -- onChange:(X)=>void FUNC(X),onCancel, where FUNC is the
  // enclosing `async function FUNC(X)` declaration. Disjoint from the three
  // above: none of them admit a space or a dot in the callee.
