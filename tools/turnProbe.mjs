...
// Extra CLI args for the launched binary, e.g. `--model fableplan
// --permission-mode plan`, to capture what a specific model/mode sends.
const EXTRA_ARGS = argOf('--args') ?? '';
// --dangerously-skip-permissions forces bypassPermissions, which would silently
// override a requested --permission-mode (e.g. plan); drop it when one is given.
const SKIP_PERMISSIONS = /--permission-mode\b/.test(EXTRA_ARGS)
  ? ''
...
const classify = body => {
  ...
  const system = Array.isArray(j.system)
    ? j.system.length
  ...
  new Promise(resolve => {
    ...
    const server = http.createServer((req, res) => {
      ...
      req.on('end', () => {
        ...
        requests.push(rec);
        if (rec.kind !== 'other')
          fs.writeFileSync(path.join(outDir, `req-${id}.json`), body);
...
const waitFor = async (predicate, timeoutMs, label) => {
  ...
  while (Date.now() < deadline) {
    pane = capture();
    if (pane.includes('[EXIT='))
      return { ok: false, pane, reason: 'process exited' };
...
const main = async () => {
  ...
  const outDir =
    argOf('--out') ??
    fs.mkdtempSync(path.join(os.tmpdir(), 'tweakcc-turnprobe-'));
  fs.mkdirSync(outDir, { recursive: true });
  const scratch = fs.mkdtempSync(
    path.join(os.tmpdir(), 'tweakcc-turnprobe-cwd-')
  );
  fs.writeFileSync(path.join(scratch, 'README.md'), '# turn probe\n');
  try {
    execFileSync('git', ['init', '-q', scratch], { stdio: 'ignore' });
  } catch {
    /* non-repo cwd is fine */
  }
  ...
  const cmd =
    `ANTHROPIC_BASE_URL=http://127.0.0.1:${port} ${JSON.stringify(bin)} ${SKIP_PERMISSIONS}${EXTRA_ARGS}; ` +
    `echo "[EXIT=$?]"; sleep 600`;
  const started = tmux(
    'new-session',
    '-d',
  ...
  if (started.status !== 0) {
    console.log(
      `turnProbe: FAIL — tmux new-session: ${started.stderr?.trim()}`
    );
    server.close();
    return 1;
  }
  ...
  try {
    const trust = await waitFor(
      p =>
        /Is this a project you created or one you trust/i.test(p) ||
        /shift\+tab to cycle/.test(p),
      90000,
      'trust dialog or composer'
    );
    if (!trust.ok) {
    ...
    if (/Is this a project you created or one you trust/i.test(trust.pane)) {
      ...
      if (!onYes) {
        note(false, 'boot: could not select "Yes, I trust this folder"');
        return 1;
      }
      await key('Enter');
    }
    const composer = await waitFor(
      p => /shift\+tab to cycle/.test(p),
      90000,
      'composer'
    );
    note(
      composer.ok,
      `boots to the composer${composer.ok ? '' : ` — ${composer.reason}`}`
    );
    if (!composer.ok) {
    ...
    const errorText = (pane.match(/.*(API Error|Error:|error).*/i) ?? [
      null,
    ])[0];
    ...
    for (const r of requests) {
      console.log(
        `    ${r.id} ${r.method} ${r.url}  ${String(r.bytes).padStart(7)} B  ${r.kind}${r.model ? `  model=${r.model}` : ''}${r.tools ? `  tools=${r.tools} system=${r.system}` : ''}  status=${r.status}`
      );
    }
    note(
      mainLoopSent(),
    ...
    const mainStatuses = requests
      .filter(r => r.kind === 'main-loop')
      .map(r => r.status);
    ...
    note(
      gotPong,
      gotPong
        ? 'reply rendered in the TUI'
        : `no reply rendered${errorText ? ` — ${errorText.trim()}` : replied.ok ? '' : ` — ${replied.reason}`}`
    );
    if (!gotPong)
      console.log(
        pane
  ...
  if (failures.length) {
    console.log(`turnProbe: FAIL — ${failures.length} check(s) failed`);
    return 1;
  }
